...
__thread int				ANNkdDim;				// dimension of space
__thread ANNpoint		ANNkdQ;					// query point
__thread double			ANNkdMaxErr;			// max tolerable squared error
__thread ANNpointArray	ANNkdPts;				// the points
__thread ANNmin_k		*ANNkdPointMK;			// set of k closest points
...
//----------------------------------------------------------------------
//	ann1Search - search for the k nearest neighbors
//----------------------------------------------------------------------

int ANNkd_tree::ann1Search(
	ANNpoint			q,				// the query point
//	int					k,				// number of near neighbors to return
//	ANNidxArray			nn_idx,			// nearest neighbor indices (returned)
//	ANNdistArray		dd,				// the approximate nearest neighbor
	ANNmin_k* ANNkdPointM1,		// create set for closest k points
	double				eps)			// the error bound
{
...
//	ANNkdPointMK = new ANNmin_k(k);		// create set for closest k points
  ANNkdPointMK = ANNkdPointM1;
  ANNkdPointMK->reset_n();
										// search starting at the root
	root->ann_search(annBoxDistance(q, bnd_box_lo, bnd_box_hi, dim));
...
//		nn_idx[i] = ANNkdPointMK->ith_smallest_info(i);
//	}
  return ANNkdPointMK->ith_smallest_info(0);
//	delete ANNkdPointMK;				// deallocate closest point set
}
